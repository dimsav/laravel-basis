...
export SHA=`find app -type f -print0  | xargs -0 shasum | shasum | awk '{print substr($0,0,40)}'`
...
then
    ...
    # Lint the code
    find -L app -name '*.php' -print0 | xargs -0 -n 1 -P 4 php -l | tee reports/phplint/.cache/phplint.$SHA.txt
fi
